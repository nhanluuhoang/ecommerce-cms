    ...
    return (
        <div key={data.id}>
            <div className='ant-select-tree-treenode ant-select-tree-treenode-switcher-open' draggable='false' aria-grabbed='false'>
                ...
                <span
                    className='ant-select-tree-node-content-wrapper ant-select-tree-node-content-wrapper-open'
                    onClick={handleOnClick}
                    itemID={data.id}
                >
                    {data.title}
            ...
            {data.children && data.children.map((item: any, index: number) => {
                if (item.children && item.children?.length > 0) {
                    ...
                    return <Item data={item} space={ newSpace + 1 } key={item.id} onSelect={handleOnClick} />
                }
            })}
        </div>
    );
...
export const TreeSelectField = ({ label, treeData, labelCol, inputCol, classRequired, registration, onSelectValue}: IProps) => {
    ...
    const handleSelectValue = (data: any) => {
        onSelectValue({
            ...
            parent_title: data.target.innerHTML
        })
    }
                            ...
                            <div>
                                <div>
                                    <div role='tree' className='ant-select-tree'>
                                        ...
                                        <div className='ant-select-tree-treenode' aria-hidden='true' style={{position: 'absolute', pointerEvents: 'none', visibility: 'hidden', height: '0px', overflow: 'hidden'}}>
                                                ...
                                                <div>
                                                    <div className='ant-select-tree-list-holder-inner' style={{display: 'flex', flexDirection: 'column'}}>
                                                        ...
                                                        {treeData && treeData.map((item, index) => {
                                                            if (item.children && item.children.length > 0) {
                                                                return <ItemHaveChild data={item} key={item.id} space={0} onSelect={handleSelectValue} />
                                                            } else {
